...
extension WindowLayoutFeatureStrings {
    static let ko = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "크기를 조절한 이웃 윈도우가 남긴 공간 채우기",
        snapFillsFreeSpaceCaption: "맞닿은 두 윈도우가 가장자리를 공유하고 하나의 크기를 조절하면, 다른 윈도우는 겹치지 않고 남은 공간을 정확히 채웁니다.",
        snapAssistEnable: "공간을 위해 다른 창 제안",
        snapAssistCaption: "창이 화면 일부에 스냅되면 남은 공간에 대해 다른 열린 창을 한 번에 하나씩 제안합니다.",
        snapAssistHint: "이 공간에 넣을 창을 선택하세요 · Esc로 건너뛰기",
        snapLinkedResizeEnable: "맞닿은 이웃 윈도우를 공유 가장자리를 따라 함께 크기 조절",
        snapLinkedResizeCaption: "그룹에 속한 윈도우의 가장자리를 드래그하면, 그 가장자리를 공유하는 이웃 윈도우도 함께 크기가 조절되어 경계가 겹치거나 벌어지지 않습니다.",
        ...
        snapLayoutsOnZoomButtonCaption: "상단 가장자리로 끄는 것과 같은 Snap Layouts 패널을 잠깐 보여줍니다. macOS가 이 버튼에 자체 메뉴를 이미 제공하는 경우 기본적으로 꺼져 있습니다.",
...
struct WindowLayoutFeatureStrings {
    ...
    let snapFillsFreeSpaceEnable: String
    let snapFillsFreeSpaceCaption: String
    let snapAssistEnable: String
    let snapAssistCaption: String
    let snapAssistHint: String
    let snapLinkedResizeEnable: String
    let snapLinkedResizeCaption: String
    ...
    static let enUS = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Fill the space a resized neighbour leaves",
        snapFillsFreeSpaceCaption: "When two snapped windows share an edge and one is resized, the other fills exactly the space left over instead of overlapping it.",
        snapAssistEnable: "Offer other windows for the space",
        snapAssistCaption: "After a window snaps into part of the screen, the other open windows are offered for what is left, one at a time.",
        snapAssistHint: "Choose a window for this space · Esc to skip",
        snapLinkedResizeEnable: "Resize a snapped neighbour along the shared edge",
        snapLinkedResizeCaption: "Drag the edge of a snapped window that borders another one in its group, and the neighbour resizes to keep the shared edge flush, without opening a gap or overlapping.",
        ...
        snapLayoutsOnZoomButtonCaption: "Briefly shows the same Snap Layouts panel dragging to the top edge does. Off by default when macOS already shows its own menu on this button.",
    ...
    static let ptBR = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Preencher o espaço deixado por uma vizinha redimensionada",
        snapFillsFreeSpaceCaption: "Quando duas janelas encaixadas compartilham uma borda e uma é redimensionada, a outra preenche exatamente o espaço que sobrou, em vez de se sobrepor.",
        snapAssistEnable: "Oferecer outras janelas para o espaço",
        snapAssistCaption: "Depois que uma janela se encaixa em parte da tela, as outras janelas abertas são oferecidas para o espaço restante, uma de cada vez.",
        snapAssistHint: "Escolha uma janela para este espaço · Esc para pular",
        snapLinkedResizeEnable: "Redimensionar uma janela vizinha encaixada ao longo da borda compartilhada",
        snapLinkedResizeCaption: "Arraste a borda de uma janela encaixada que faz fronteira com outra do mesmo grupo, e a vizinha é redimensionada para manter a borda compartilhada alinhada, sem abrir um vão nem se sobrepor.",
        ...
        snapLayoutsOnZoomButtonCaption: "Mostra rapidamente o mesmo painel de Snap Layouts que arrastar até a borda superior. Desativado por padrão quando o macOS já mostra o próprio menu nesse botão.",
    ...
    static let tr = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Yeniden boyutlandırılan komşunun bıraktığı boşluğu doldur",
        snapFillsFreeSpaceCaption: "İki yerleştirilmiş pencere bir kenarı paylaştığında ve biri yeniden boyutlandırıldığında, diğeri üst üste binmek yerine kalan boşluğu tam olarak doldurur.",
        snapAssistEnable: "Boşluk için diğer pencereleri öner",
        snapAssistCaption: "Bir pencere ekranın bir bölümüne yerleştirildiğinde, kalan boşluk için diğer açık pencereler tek tek önerilir.",
        snapAssistHint: "Bu boşluk için bir pencere seçin · Atlamak için Esc",
        snapLinkedResizeEnable: "Yerleştirilmiş komşuyu paylaşılan kenar boyunca yeniden boyutlandır",
        snapLinkedResizeCaption: "Grubundaki başka bir pencereyle sınır paylaşan yerleştirilmiş bir pencerenin kenarını sürükleyin; komşu pencere, aralarında boşluk veya çakışma oluşmadan paylaşılan kenarı hizalı tutacak şekilde yeniden boyutlandırılır.",
        ...
        snapLayoutsOnZoomButtonCaption: "Üst kenara sürüklemekle aynı Snap Layouts panelini kısaca gösterir. macOS bu düğmede zaten kendi menüsünü gösteriyorsa varsayılan olarak kapalıdır.",
    ...
    static let ru = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Заполнять пространство, оставшееся после изменения размера соседнего окна",
        snapFillsFreeSpaceCaption: "Когда два привязанных окна имеют общий край и одно из них изменяет размер, другое заполняет ровно оставшееся пространство, а не перекрывает его.",
        snapAssistEnable: "Предлагать другие окна для свободного места",
        snapAssistCaption: "После того как окно привязывается к части экрана, для оставшегося места по одному предлагаются другие открытые окна.",
        snapAssistHint: "Выберите окно для этого места · Esc, чтобы пропустить",
        snapLinkedResizeEnable: "Изменять размер привязанного соседнего окна вдоль общей границы",
        snapLinkedResizeCaption: "Перетащите край привязанного окна, граничащий с другим окном той же группы, и соседнее окно изменит размер так, чтобы общая граница оставалась ровной — без зазора и без перекрытия.",
        ...
        snapLayoutsOnZoomButtonCaption: "Кратко показывает ту же панель Snap Layouts, что и перетаскивание к верхнему краю. По умолчанию выключено, если macOS уже показывает на этой кнопке собственное меню.",
    ...
    static let es = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Llenar el espacio que deja una ventana vecina redimensionada",
        snapFillsFreeSpaceCaption: "Cuando dos ventanas ajustadas comparten un borde y una se redimensiona, la otra llena exactamente el espacio restante en lugar de superponerse.",
        snapAssistEnable: "Ofrecer otras ventanas para el espacio",
        snapAssistCaption: "Cuando una ventana se ajusta a una parte de la pantalla, las demás ventanas abiertas se ofrecen para el espacio restante, una a la vez.",
        snapAssistHint: "Elige una ventana para este espacio · Esc para omitir",
        snapLinkedResizeEnable: "Redimensionar una ventana vecina ajustada a lo largo del borde compartido",
        snapLinkedResizeCaption: "Arrastra el borde de una ventana ajustada que colinda con otra de su grupo, y la vecina se redimensiona para mantener el borde compartido alineado, sin dejar un hueco ni superponerse.",
        ...
        snapLayoutsOnZoomButtonCaption: "Muestra brevemente el mismo panel de Snap Layouts que arrastrar al borde superior. Desactivado de forma predeterminada cuando macOS ya muestra su propio menú en este botón.",
    ...
    static let de = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Den Platz füllen, den ein verkleinertes Nachbarfenster freigibt",
        snapFillsFreeSpaceCaption: "Wenn zwei eingerastete Fenster einen Rand teilen und eines in der Größe geändert wird, füllt das andere genau den verbleibenden Platz, statt sich zu überlappen.",
        snapAssistEnable: "Andere Fenster für den Platz anbieten",
        snapAssistCaption: "Nachdem ein Fenster in einen Bildschirmbereich einrastet, werden die anderen geöffneten Fenster nacheinander für den verbleibenden Platz angeboten.",
        snapAssistHint: "Wähle ein Fenster für diesen Platz · Esc zum Überspringen",
        snapLinkedResizeEnable: "Eingerastetes Nachbarfenster entlang der gemeinsamen Kante mit anpassen",
        snapLinkedResizeCaption: "Ziehe den Rand eines eingerasteten Fensters, der an ein anderes Fenster derselben Gruppe grenzt, und das Nachbarfenster wird in der Größe angepasst, damit die gemeinsame Kante bündig bleibt, ohne Lücke oder Überlappung.",
        ...
        snapLayoutsOnZoomButtonCaption: "Zeigt kurz dasselbe Snap-Layouts-Panel wie das Ziehen an den oberen Rand. Standardmäßig aus, wenn macOS an diesem Knopf bereits ein eigenes Menü anzeigt.",
    ...
    static let fr = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Remplir l’espace laissé par une fenêtre voisine redimensionnée",
        snapFillsFreeSpaceCaption: "Lorsque deux fenêtres ancrées partagent un bord et que l’une est redimensionnée, l’autre remplit exactement l’espace restant au lieu de le chevaucher.",
        snapAssistEnable: "Proposer les autres fenêtres pour l’espace",
        snapAssistCaption: "Une fois qu’une fenêtre s’ancre dans une partie de l’écran, les autres fenêtres ouvertes sont proposées pour l’espace restant, une à la fois.",
        snapAssistHint: "Choisissez une fenêtre pour cet espace · Échap pour ignorer",
        snapLinkedResizeEnable: "Redimensionner une fenêtre voisine ancrée le long du bord partagé",
        snapLinkedResizeCaption: "Faites glisser le bord d’une fenêtre ancrée qui touche une autre fenêtre du même groupe, et la voisine est redimensionnée pour garder ce bord commun aligné, sans espace ni chevauchement.",
        ...
        snapLayoutsOnZoomButtonCaption: "Affiche brièvement le même panneau Snap Layouts que le glisser vers le bord supérieur. Désactivé par défaut quand macOS affiche déjà son propre menu sur ce bouton.",
    ...
    static let it = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "Riempi lo spazio lasciato da una finestra vicina ridimensionata",
        snapFillsFreeSpaceCaption: "Quando due finestre allineate condividono un bordo e una viene ridimensionata, l’altra riempie esattamente lo spazio rimasto invece di sovrapporsi.",
        snapAssistEnable: "Proponi le altre finestre per lo spazio libero",
        snapAssistCaption: "Dopo che una finestra si aggancia a una parte dello schermo, le altre finestre aperte vengono proposte per lo spazio rimasto, una alla volta.",
        snapAssistHint: "Scegli una finestra per questo spazio · Esc per saltare",
        snapLinkedResizeEnable: "Ridimensiona una finestra vicina allineata lungo il bordo condiviso",
        snapLinkedResizeCaption: "Trascina il bordo di una finestra allineata che confina con un’altra dello stesso gruppo: la vicina viene ridimensionata per mantenere il bordo condiviso allineato, senza spazi vuoti né sovrapposizioni.",
        ...
        snapLayoutsOnZoomButtonCaption: "Mostra brevemente lo stesso pannello Snap Layouts del trascinamento verso il bordo superiore. Disattivato per impostazione predefinita quando macOS mostra già un proprio menu su questo pulsante.",
    ...
    static let ja = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "サイズ変更した隣のウインドウが残した分だけ埋める",
        snapFillsFreeSpaceCaption: "スナップした2つのウインドウが辺を共有していて片方のサイズを変えると、もう片方は重ならずに残ったスペースぴったりに埋まります。",
        snapAssistEnable: "残りのスペースに他のウインドウを提案",
        snapAssistCaption: "ウインドウが画面の一部にスナップされた後、残りのスペースに他の開いているウインドウが1つずつ提案されます。",
        snapAssistHint: "このスペースに入れるウインドウを選択 · Escでスキップ",
        snapLinkedResizeEnable: "共有する境界に沿ってスナップした隣のウインドウもサイズ変更",
        snapLinkedResizeCaption: "同じグループの別のウインドウと接しているスナップ済みウインドウの境界をドラッグすると、隣のウインドウも境界がぴったり合うようにサイズが変わり、隙間も重なりもできません。",
        ...
        snapLayoutsOnZoomButtonCaption: "上端へのドラッグと同じSnap Layoutsパネルを短く表示します。macOSがこのボタンに独自のメニューをすでに表示する場合は、既定でオフになります。",
    ...
    static let zhHans = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "填充调整大小后的相邻窗口留出的空间",
        snapFillsFreeSpaceCaption: "当两个已贴靠的窗口共用一条边、其中一个被调整大小后，另一个会精确填充剩余空间，而不是与其重叠。",
        snapAssistEnable: "为剩余空间推荐其他窗口",
        snapAssistCaption: "窗口贴靠到屏幕的一部分后，会依次为剩余空间推荐其他已打开的窗口。",
        snapAssistHint: "为此空间选择一个窗口 · 按 Esc 跳过",
        snapLinkedResizeEnable: "沿共用边缘调整已贴靠相邻窗口的大小",
        snapLinkedResizeCaption: "拖动某个已贴靠窗口与同组其他窗口相邻的边缘，相邻窗口会随之调整大小，让共用边缘保持齐平，既不留缝隙也不重叠。",
        ...
        snapLayoutsOnZoomButtonCaption: "会短暂显示与拖到屏幕顶部边缘相同的 Snap Layouts 面板。当 macOS 已在此按钮上显示自己的菜单时，默认关闭。",
    ...
    static let zhTW = WindowLayoutFeatureStrings(
        ...
        snapFillsFreeSpaceEnable: "填滿調整大小後相鄰視窗留下的空間",
        snapFillsFreeSpaceCaption: "當兩個已貼齊的視窗共用一個邊緣，其中一個被調整大小時，另一個會精確填滿剩餘空間，而不是與它重疊。",
        snapAssistEnable: "為剩餘空間推薦其他視窗",
        snapAssistCaption: "視窗貼齊螢幕的一部分後，會依序為剩餘空間推薦其他已開啟的視窗。",
        snapAssistHint: "為此空間選擇一個視窗 · 按 Esc 跳過",
        snapLinkedResizeEnable: "沿共用邊緣調整已貼齊相鄰視窗的大小",
        snapLinkedResizeCaption: "拖曳某個已貼齊視窗與同群組中其他視窗相鄰的邊緣，相鄰視窗會隨之調整大小，讓共用邊緣保持齊平，不留空隙也不重疊。",
        ...
        snapLayoutsOnZoomButtonCaption: "會短暫顯示與拖曳到螢幕頂端邊緣相同的 Snap Layouts 面板。當 macOS 已在此按鈕上顯示自己的選單時，預設為關閉。",
    ...
    static let zhHK = WindowLayoutFeatureStrings(
        ...
        snapLayoutsOnZoomButtonCaption: "會短暫顯示同拖到螢幕頂邊一樣嘅 Snap Layouts 面板。當 macOS 已經喺呢個掣度顯示自己嘅選單時，預設就係關閉。",
