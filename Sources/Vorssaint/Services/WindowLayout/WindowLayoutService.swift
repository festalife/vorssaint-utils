...
final class WindowLayoutService: ObservableObject {
    ...
    private init() {
        ...
        SnapController.shared.host = self
    }
    ...
    func syncWithPreferences() {
        ...
        SnapController.shared.syncWithPreferences()
    }
    ...
    func apply(_ action: WindowLayoutAction) -> WindowLayoutResult {
        ...
        if action == .restore {
            ...
            if setFrame(previous, on: target.window, windowKey: target.key) {
                ...
                SnapController.shared.windowLeftGroups(target.windowID, reason: "restored to its previous frame")
        ...
        if action == .fullScreen {
            ...
            if applied {
                ...
                SnapController.shared.windowLeftGroups(target.windowID, reason: "went full screen")
            }
        ...
        if action == .previousDisplay || action == .nextDisplay {
            ...
            if setFrame(axFrame(fromAppKit: rect),
                ...
                SnapController.shared.windowLeftGroups(target.windowID, reason: "moved to another display")
        ...
        if let crossing = WindowLayoutGeometry.displayCrossing(for: action,
           ...
           let destination = sidewaysScreen(to: screen,
            ...
            SnapController.shared.windowLeftGroups(target.windowID, reason: "crossed to the adjacent display")
    ...
    /// The one place a Window Layout action becomes a window frame.
    ///
    /// A placement that finds the window already exactly on its zone is still
    /// a placement: it records the action, updates the Snap Group and runs the
    /// Snap Assist decision, exactly like one that had to move the window.
    /// Returning early there — the old behaviour — is what made the overlay
    /// appear the first time a window was snapped left and not the second,
    ...
    private func applyPlacement(_ action: WindowLayoutAction,
                                ...
                                origin: SnapPlacementOrigin = .user) -> WindowLayoutResult {
        let currentRect = appKitFrame(fromAX: target.frame)
        ...
        let alreadyInPlace = placement.frame == target.frame
        if !alreadyInPlace {
            frameHistory.record(historyFrame ?? target.frame, for: target.key)
            guard setFrame(placement.frame,
    ...
    private func placement(for action: WindowLayoutAction,
        ...
        let adjusted = SnapController.shared.freeSpace(for: action,
                                                       theoreticalZone: rect,
                                                       visibleFrame: visibleFrame,
                                                       fallbackRect: appKitFrame(fromAX: current),
                                                       excluding: windowID)
        let integral = adjusted.integral
        return WindowLayoutPlacement(frame: axFrame(fromAppKit: integral), rect: integral)
    }

    /// Dock Preview and the Switcher ask this to offer "restore the whole
    /// group"; the membership itself lives in `SnapController`.
    func snapGroupPeers(of windowID: CGWindowID) -> [(windowID: CGWindowID, pid: pid_t)]? {
        SnapController.shared.snapGroupPeers(of: windowID)
    }
    ...
    private func observeDirectionalEvent(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if type == .scrollWheel {
            ...
            if scrollY > 0.5 {
                ...
                SnapController.shared.showZonePreview(frame: preview, action: .maximize)
            } else if scrollY < -0.5 {
                session.manualOverride = .minimize
                directionalSession = session
                updateDirectionalIndicator(action: .minimize)
                SnapController.shared.hideZonePreview(immediately: true)
            }
            return nil
        }
    ...
    private func updateDirectionalGesture() {
        ...
        guard let action else {
            SnapController.shared.hideZonePreview(immediately: false)
            return
        }
        ...
        if let layoutAction = action.layoutAction {
            ...
            SnapController.shared.showZonePreview(frame: preview, action: layoutAction)
        } else {
            SnapController.shared.hideZonePreview(immediately: true)
        }
    }
    ...
    private func startEdgeSnapTap() {
        ...
        CFRunLoopAddSource(CFRunLoopGetMain(), source, .commonModes)
        CGEvent.tapEnable(tap: tap, enable: true)
        SnapController.shared.tapStarted()
    }

    private func stopEdgeSnapTap() {
        SnapController.shared.tapStopped()
    ...
    /// The tap callback. It runs on the main run loop (the tap's source is
    /// added there), copies the scalars it needs and hands every decision to
    /// `SnapController` on a later turn — no Accessibility work ever happens
    /// inside an event callback.
    ///
    /// The one thing it does change is a drag event's own location, and only
    /// to keep the physical top edge from tripping macOS's window overview:
    /// `adjustedDragLocation` reads cached scalars and touches nothing else.
    private func observeEdgeSnapEvent(type: CGEventType,
        ...
        if type == .tapDisabledByTimeout || type == .tapDisabledByUserInput {
            ...
            DispatchQueue.main.async { SnapController.shared.abortSequence(reason: "event tap disabled") }
            return Unmanaged.passUnretained(event)
        }
        ...
        switch type {
        case .mouseMoved:
            DispatchQueue.main.async { SnapController.shared.handlePointerMoved(atQuartz: location) }
        case .leftMouseDown:
            let flags = event.flags
            DispatchQueue.main.async {
                SnapController.shared.handlePointerClickForZoomHover(atQuartz: location, isDown: true)
                guard SnapController.shared.beginPointerSequence() else { return }
                SnapController.shared.handlePointerDown(atQuartz: location, flags: flags)
            }
        case .leftMouseDragged:
            event.location = SnapController.shared.adjustedDragLocation(location)
            DispatchQueue.main.async { SnapController.shared.handlePointerDragged(atQuartz: location) }
        case .leftMouseUp:
            DispatchQueue.main.async {
                SnapController.shared.handlePointerClickForZoomHover(atQuartz: location, isDown: false)
                guard !SnapController.shared.isSequenceSuppressed else {
                    SnapController.shared.endSuppressedSequence()
                    return
                }
                SnapController.shared.handlePointerUp(atQuartz: location)
            }
        default:
            break
        }
...
// MARK: - Snap placement host

/// The whole surface `SnapController` uses to move a window: resolving a
/// target and writing a placement, both through this service's own settling
/// and recovery path. There is deliberately no second AX mutation algorithm.
extension WindowLayoutService: SnapPlacementHost {
    func snapFocusedTarget(for action: WindowLayoutAction) -> WindowLayoutTarget? {
        focusedTarget(for: action)
    }
    ...
    func snapTarget(windowID: CGWindowID,
        ...
        guard let window = SnapAX.window(windowID, in: axApp, timeout: SnapAX.Timeout.focused) else { return nil }
        // A window just brought back from minimized has not necessarily
        // reported itself on screen yet, so a Snap Assist pick asks for the
        // fuller set rather than the on-screen one.
        let known = allowOffScreen ? SnapAX.allWindowIDs() : (onScreenWindowIDs() ?? [])
        return target(from: window, app: app, onScreenWindowIDs: known, capability: capability)
    }
    ...
    func snapApplyPlacement(_ action: WindowLayoutAction,
        ...
        pruneWindowState(keeping: target.key)
        let result = applyPlacement(action,
                                    to: target,
                                    visibleFrame: visibleFrame,
                                    historyFrame: historyFrame,
                                    cyclesRepeatedAction: cyclesRepeatedAction,
                                    origin: origin)
